...
sub do_foreach {
    BenchmarkJavaScriptPacker::set_foreach();
    BenchmarkRegGrp::set_foreach();

    do_stuff();
}

sub do_map {
    BenchmarkJavaScriptPacker::set_map();
    BenchmarkRegGrp::set_map();
...
sub do_stuff {
    ...
    $packer->minify( \$in, { compress => 'clean' } );
